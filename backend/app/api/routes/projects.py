    ...
    for c in clips:
        ...
        clip_responses.append(
            RenderedClipResponse(
                ...
                mirror_video=getattr(c, "mirror_video", False) or False,
                anti_copyright=getattr(c, "anti_copyright", False) or False,
                video_scale=float(getattr(c, "video_scale", 1.0) or 1.0),
                video_pan_x=float(getattr(c, "video_pan_x", 0.0) or 0.0),
                video_pan_y=float(getattr(c, "video_pan_y", 0.0) or 0.0),
...
async def process_project(
    ...
    job = Job(
        ...
        mirror_video=bool(getattr(req, "mirror_video", False)),
        anti_copyright=bool(getattr(req, "anti_copyright", False)),
        video_scale=float(getattr(req, "video_scale", 1.0) or 1.0),
        video_pan_x=float(getattr(req, "video_pan_x", 0.0) or 0.0),
        video_pan_y=float(getattr(req, "video_pan_y", 0.0) or 0.0),
        custom_min_duration=getattr(req, "custom_min_duration", None),
        custom_max_duration=getattr(req, "custom_max_duration", None),
        status="queued",
        current_stage=1,
